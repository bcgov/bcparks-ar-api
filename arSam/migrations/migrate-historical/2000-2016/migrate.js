...
const {
  ...
  clientIDsAR
} = require('../legacy-data-functions');
const { schema } = require('../legacy-data-constants');

const MAX_TRANSACTION_SIZE = 25;

// Change file path based on year(s) being uploaded
let filePath = 'ar-records-2003-2005.xlsx';
...
async function run() {
  ...
  try {
    if (process.argv.length <= 2) {
      ...
      console.log('');
      console.log('Usage: node migrate.js <env>');
      console.log('');
      console.log('Options');
  ...
  try {
    ...
    for (const row of rows) {
      ...
      try {
        ...
        failures.push({
          item: { data: row },
          reason: `Error collecting activities from historical data. ${error}`,
          error: error
        });
        continue;
      }
      ...
      try {
        ...
        failures.push({
          item: { data: row },
          reason: `An error occurred while creating a legacy park: ${error}`,
          error: error
        });
        continue;
      }
      ...
      try {
        ...
        if (xdb[row.legacy_orcs]) {
          existingSubArea = Object.keys(xdb[row.legacy_orcs]).filter(
            (id) => xdb[row.legacy_orcs]?.[id].subAreaName === checkSubAreaName
          );
        ...
        if (!existingSubArea.length) {
          existingSubArea = Object.keys(newSubAreas).filter((id) => newSubAreas?.[id].subAreaName === checkSubAreaName);
          subAreaId = existingSubArea[0] ? existingSubArea[0] : null;
        }
        ...
        failures.push({
          item: { data: row },
          reason: `An error occurred while creating a legacy subarea: ${error}`,
          error: error
        });
        continue;
      }
      ...
      if (!subAreaId) {
        ...
        failures.push({
          item: { data: row },
          reason: `An error occurred determining a subarea id for the historical data`
        });
        continue;
      }
    ...
    console.log(
      'Please review and approve the following change log. The migration will commence once the change log is approved.'
    );
    ...
    if (Object.keys(newParks).length) {
      let viewParks = await getConsoleInput(
        `There are ${Object.keys(newParks).length || 0} legacy parks to be created.\nDo you want to review these parks? [Y/N] >>> `
      );
      process.stdout.write('\n');
    ...
    if (Object.keys(newSubAreas).length) {
      let viewSubAreas = await getConsoleInput(
        `There are ${Object.keys(newSubAreas).length || 0} legacy subareas to be created.\nDo you want to review these subareas? [Y/N] >>> `
      );
      process.stdout.write('\n');
    ...
    if (failures.length) {
      process.stdout.write('\n');
      let viewErrors = await getConsoleInput(
        `There were ${failures.length} rows containing errors that will prevent their contained data from being migrated.\nThey will not be migrated, but they will be captured for future processing.\nDo you want to review these failures? [Y/N] >>> `
      );
      process.stdout.write('\n');
      if (viewErrors === 'Y' || viewErrors === 'y') {
        ...
        for (const failure of failures) {
          ...
          let index = failureList.findIndex((item) => item.string == failureString);
          if (index !== -1) {
...
async function executeChangelog() {
  ...
  if (Object.keys(newParks).length) {
    try {
      ...
      let intervalStartTime = new Date().getTime();
      let legacyParksList = Object.keys(newParks).map((orcs) => newParks[orcs]);
      let parkTransactChunk = { TransactItems: [] };
      try {
        for (const park of legacyParksList) {
          updateConsoleProgress(
            intervalStartTime,
            'Creating legacy parks transaction',
            legacyParksList.indexOf(park) + 1,
            legacyParksList.length,
            1
          );
      ...
      try {
        ...
        for (const transaction of parkTransactObjList) {
          updateConsoleProgress(
            intervalStartTime,
            'Executing legacy parks transaction',
            parkTransactObjList.indexOf(transaction) + 1,
            parkTransactObjList.length,
            1
          );
          await dynamoClient(new TransactWriteItemsCommand(transaction));
        }
      } catch (error) {
  ...
  if (Object.keys(newSubAreas).length) {
    ...
    let intervalStartTime = new Date().getTime();
    let legacySubAreasList = Object.keys(newSubAreas).map((id) => newSubAreas[id]);
    ...
    try {
      for (const subArea of legacySubAreasList) {
        updateConsoleProgress(
          intervalStartTime,
          'Creating legacy subarea transaction',
          legacySubAreasList.indexOf(subArea) + 1,
          legacySubAreasList.length,
          1
        );
        if (
          subAreaTransactChunk.TransactItems.length + 2 > MAX_TRANSACTION_SIZE ||
          touchedParksList.indexOf(subArea.orcs) !== -1
        ) {
        ...
        let subAreaObj = {
          TableName: TABLE_NAME,
          Item: Object.assign(marshall(subArea), { activities: { SS: subArea.activities } }),
          ConditionExpression: 'attribute_not_exists(pk) AND attribute_not_exists(sk)'
        };
        subAreaTransactChunk.TransactItems.push({
          Put: subAreaObj
  ...
  try {
    for (const item of changes) {
      try {
        ...
        updateConsoleProgress(
          intervalStartTime,
          'Creating legacy record transaction',
          changes.indexOf(item) + 1,
          changes.length,
          100
        );
  ...
  for (const transaction of transactionMap) {
    try {
      updateConsoleProgress(
        intervalStartTime,
        'Executing legacy record transaction',
        transactionMap.indexOf(transaction) + 1,
        transactionMap.length,
        10
      );
      await dynamoClient.send(new TransactWriteItemsCommand(transaction.transactions));
      ...
      for (const row of transaction.rows) {
        failures.push({
          item: row,
          reason: `This row failed as part of an aborted bulk transaction and might not include any errors: ${error}`,
          error: error
        });
      }
    }
  ...
  let saveAreas = await getConsoleInput(
    `Do you want to save lists of the successfully created parks and subarea [Y/N] >>> `
  );
  ...
  // Review successes.
  let saveSuccesses = await getConsoleInput(
    `Do you want to save a list of the successfully created legacy records? [Y/N] >>> `
  );
  ...
  // Review failures.
  let saveFailures = await getConsoleInput(
    `Do you want to save a list of the items that failed to migrate? [Y/N] >>> `
  );
