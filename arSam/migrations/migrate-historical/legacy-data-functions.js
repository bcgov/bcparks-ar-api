...
const { getParks, getSubAreas } = require('../../layers/baseLayer/baseLayer');
...
function updateConsoleProgress(intervalStartTime, text, complete = 0, total = 1, modulo = 1) {
  if (complete % modulo === 0 || complete === total) {
    ...
    process.stdout.write(
      ` ${text}: ${complete}/${total} (${percent.toFixed(1)}%) completed in ${formatTime(currentElapsed)} (~${formatTime(remainingTime)} remaining)\r`
    );
  }
}
...
function determineActivities(row, schema) {
  ...
  for (const entry in schema) {
    ...
    if (activityList.indexOf(activity) === -1 && activity !== 'Meta') {
      if (type === String && prop && prop !== '' && prop !== '0') {
        // valid string field
...
function createLegacySubAreaObject(data, id, subAreaName, activities) {
  try {
    ...
    throw (
      `Failed to create legacy subarea object ${String(id).padStart(4, '0')} (${data?.legacy_parkSubArea}): ` + error
    );
  }
}
