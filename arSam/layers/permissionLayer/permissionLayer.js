...
function verifySecret(tokenString, secret, callback, sendError) {
  jwt.verify(tokenString, secret, function (verificationError, decodedToken) {
    ...
    if (verificationError == null && decodedToken && decodedToken.resource_access["attendance-and-revenue"] && decodedToken.resource_access["attendance-and-revenue"].roles) {
      ...
      logger.debug('decodedToken roles', decodedToken.resource_access["attendance-and-revenue"].roles);
...
exports.resolvePermissions = function (token) {
  ...
  try {
    ...
    roles = token.data.resource_access['attendance-and-revenue'].roles;
    // If we get here, they have authenticated and have some roles in the attendance-and-revenue client.  Treat them as
