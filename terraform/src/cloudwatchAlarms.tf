resource "aws_lambda_function" "cloudwatch_alarm" {
  function_name = "cloudwatchAlarm-${random_string.postfix.result}"
...
data "aws_iam_policy_document" "sns-topic-policy" {
  ...
  statement {
    ...
    principals {
      type        = "AWS"
      identifiers = [aws_iam_role.parkRole.arn]
    }
...
resource "aws_sqs_queue" "alarm_queue" {
  name                      = "cloudwatch-alarm-queue${random_string.postfix.result}"
